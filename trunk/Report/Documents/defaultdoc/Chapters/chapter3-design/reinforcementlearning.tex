...
\begin{flushleft}
$\alpha$  - Is the learning rate, which basically means how much you modify the value of the coefficient $\theta_i$ to fit the current example or situation; how much it learns from each visit to every state. It is a number, $0 < \alpha < 1$, that in a normal temporal difference equation (REFERENCE!!!!!) would (optimaly) decrease according to how many times a state is visited. Since every one of our states is visited inifinitely many times, the value $\alpha$ can be a fixed number that we modify manually. The higher the value, the more you learn every specific case.
\end{flushleft} 
