...
\end{Sourcecode}
	...
	\lnnum{2} is a small calculation also found in section \ref{SCA_label} called $due - de + due$. The point of this calculation is make tiles closer to the enemy have a higher potential. If we just used the distance from the enemy to the tile, then the tiles facing away from the unit would have a higher potential, even though we actually want to be closer. The potential by using $due - de + due$ can be observed in figure \ref{fig:MDP}.
