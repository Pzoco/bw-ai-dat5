...
We chose to calculate new weights or coefficients for the $\hat{Q}_f$ function every $n$ frames. Trying to update for every single one created a lag in the performance of the game. The optimization we created for updating while still preserving the game speed is the following: 
\pagebreak
