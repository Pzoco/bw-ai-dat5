\section{Functions for the potential fields}
	...
	$da =$ distance from unit tile to nerest ally unit.\\
	$dua =$ distance from current tile to nerest ally unit.\\
	...
	$dc =$ distance from unit tile to nerest cliff or edge. \\
	$duc =$ distance from current tile to nerest cliff or edge. 
	...
	\subsection*{Squad center (Attractive)}
	    ...
	    The first case handles then whe unis are within the desired squad size. In that case we dont need to attract the units more towards the squad center.
	    
	    In the secoung case, the unit are outside the desired squad size, in that case we need to attract it towards the squad center.
	...
	\subsection*{Maximum distance positioning (Attractive)}
		...
		In the first case handle the case there se want to get closer to the enemies because we are not within shooting range.
		In the secound case the unit is within range and dont need to get closer. One could argue that we need a third case to handle if we are getting to close, but this is handled by \textit{Weapon cool down}.
...
We do not want all of the vultures to clump completely together during a match. This can lead them vulnerable to splash damage attacks. If we keep the vulture spread out a reasonable distance the splash damage will not take any effect.
	...
	The first case handles when the unit is fare enough away from the enemies to not substain splash damage, so we dont need to get further away.
